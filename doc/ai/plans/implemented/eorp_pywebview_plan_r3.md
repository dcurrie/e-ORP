# e-ORP: Conversion Plan — Jupyter/ipywidgets → PyWebView (Revision 3)
...
release and commit it to `frontend/`. Reference it in `index.html` as a relative
path.

**Plotly JS version:** Python plotly 6.x corresponds to plotly.js 3.x — the two
projects synchronized their major version numbers at the 6.0/3.0 release (R2
incorrectly stated v2.x). To get the exact bundled JS version for a given Python
release, after installing the package inspect `plotly/package_data/plotly.min.js`
— the version string appears in the file header. For `plotly==6.1.0` run:
`python -c "import plotly; import os; print(open(os.path.join(os.path.dirname(plotly.__file__), 'package_data', 'plotly.min.js')).read(80))"` to read the header. Use that exact file as `frontend/plotly.min.js` to guarantee the Python and JS rendering engines match.
...
def render_dd(dd, fname=None):
    ...
    Replaces display_dd. Returns a list of output items:
      {'type': 'plotly',  'json': <dict>}   # figure as a Python dict, NOT a string
      {'type': 'table',   'html': <str>}
      {'type': 'heading', 'text': <str>}

    fig.to_json() returns a JSON string. json.loads() converts it to a Python
    dict so that when the items list is later serialized with json.dumps() the
    figure is embedded as a nested object, not a string. This means item.json
    arrives in JavaScript as an object with .data and .layout properties directly
    accessible — JSON.parse() is not needed on the JS side.
    """
    items = []
...
// parseInt(val, 10) is safe for all INT_PARAMS: none of them are in STR_PARAMS,
// so they never receive empty strings. For decimal strings like "65.0" from
// number inputs, parseInt truncates cleanly to 65, matching the backend's
// int(float(val)) pattern. Math.trunc(Number(val)) is equivalent here.
...
function renderResults(items) {
    ...
    items.forEach((item, idx) => {
        if (item.type === 'plotly') {
            ...
            // item.json is a plain JS object (not a string) because renderer.py
            // stores json.loads(fig.to_json()) — a dict — which json.dumps embeds
            // as a nested object literal. .data and .layout are directly accessible.
...
**Note on `renderResults` JSON handling:** `fig.to_json()` in Plotly returns a
JSON **string**. Storing that string directly in the items list would mean
`json.dumps(items)` embeds it as a JSON string value, and `item.json` in
JavaScript would be a string — `item.json.data` and `item.json.layout` would
both be `undefined`, silently breaking all charts.

The fix is in `renderer.py`: wrap each figure with `json.loads(fig.to_json())`
to convert the string to a Python dict before appending. When `json.dumps(items)`
then serializes the list, the figure is emitted as a nested JSON object, and
`item.json` arrives in JavaScript as an object with `.data` and `.layout`
properties directly accessible. `JSON.parse()` is not needed on the JS side.

The `evaluate_js` call pattern `f'renderResults({json.dumps(items)})'` is correct
and safe: `json.dumps` produces a valid JSON value (an array of objects), which
is also valid JavaScript syntax as a function argument. No string-escaping issues
arise from this pattern.
...
2. Verify Plotly JSON round-trip: `json.loads(fig.to_json())` produces a dict
   whose `data` and `layout` keys are directly usable as
   `Plotly.newPlot(div, fig_obj.data, fig_obj.layout)` in the browser.
