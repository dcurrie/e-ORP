...
1. In each bracket table, **insert a new row above the last row**, then copy the
   last row into the new row and update the values. Do not append to the end of
   the table — Numbers only auto-expands INDEX ranges for rows inserted in the
   middle, not rows appended at the end.
2. Add a new row to the Standard Deductions table using the same insert-then-copy
   technique.
3. Update `LastAdjYear` in Global Parameters to the new year.
4. Verify that INDEX ranges in TaxCalc helper cells still cover all data rows —
   if any range was not auto-expanded, extend it manually.
